        ...
        <div class="card" :style="`background-color:${item.background};`" :class="item.class">
            <a :href="item.url" :target="item.target" rel="noreferrer">
                <div class="card-content p-0 pl-4" style="display: flex; align-items: center;">
                    ...
                    <div class="columns is-mobile" :style="[item.tag ? 'margin-bottom: -10px;' : '']">
                        ...
                        <div class="column">
                            <slot name="content">
                                <p class="title is-4 mb-1" :class="[stats ? 'm-0' : '']">{{ item.name }}</p>
                                <template v-if="stats.length > 0">
                                    <p v-for="stat in stats" class="subtitle is-6 m-0" style="font-size: small;">
...
export default {
    ...
    async created() {
        ...
        // Send a message to the socket server to register this app and listen for updates
        this.$socket.emit("service", { id: this.item.id });
        this.getStatus();
    ...
    methods: {
        async getStatus() {
            const data = await fetch(`/api/apps/${this.item.id}/stats`);
